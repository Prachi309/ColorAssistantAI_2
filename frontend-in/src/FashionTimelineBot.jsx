...
function extractTimelineDays(aiText) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (dayRegex.test(line.replace(/\*/g, '').trim())) {
      ...
      if (line.toLowerCase().includes('outfit:') || line.toLowerCase().includes('top:')) {
        // Collect all lines until next day or empty line
        let outfitLines = [line.replace(/[#*]/g, '').replace(/Outfit:/i, '').trim()];
        let j = i + 1;
        while (j < lines.length && lines[j] && !dayRegex.test(lines[j].replace(/\*/g, '').trim())) {
          outfitLines.push(lines[j].replace(/[#*]/g, '').trim());
          j++;
        }
        currentDay.outfit = outfitLines.join(' ').replace(/\s+/g, ' ');
      }
    }
  }
  if (currentDay) {
  ...
  const handleNumberSubmit = async () => {
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        const days = extractTimelineDays(suggestion);
